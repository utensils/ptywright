...
5. **Does background task state ever reach the classifier as a top-level state worth detecting?**
   - E.g. "task X failed and is blocking the prompt." If so, we'd add a new classifier state to the existing set advertised by `plugin.describe` (currently including `starting`, `waiting_for_trust`, `waiting_for_enter_plan_mode`, `waiting_for_plan_approval`, `waiting_for_permission`, `waiting_for_external_editor`, `usage_screen`, `local_ui_screen`, `thinking`, `cancelling`, `completed_turn`, `error`, `ready`, and `waiting_for_user_input` — see `plugins/claude-code/main.lua`).
