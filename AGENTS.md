...
**`repl` feature** (interactive REPL client, `ptywright repl`) is on by default. To verify the lean build without `ratatui` / `tui-input` / `crossbeam-channel` / `nu-ansi-term`:
...
- REPL client (gated `#[cfg(feature = "repl")]`, on by default — opt out with `cargo build --no-default-features`):
  - `src/repl/mod.rs` — public entry (`ReplArgs`, `Transport`, `Framing`, `run`) invoked from `Commands::Repl`. Builds the right transport, holds the stdio child guard for the REPL's lifetime, hands off to the ratatui event loop in `tui.rs`.
  ...
  - `src/repl/ctx.rs` — shared `ReplCtx` (tabs, focus) used by the dispatcher, the App state, and the dispatcher thread.
  - `src/repl/completer.rs`, `src/repl/highlighter.rs`, `src/repl/history.rs` — TUI-agnostic helpers: a `complete(line, pos) -> Vec<Suggestion>` engine over the static DSL table + cached plugin names + live adapter ids; a ratatui-styled DSL highlighter (`highlight(line) -> Vec<Span>`); and a `ReplHistory` file-backed `VecDeque<String>` rooted at `Paths::repl_history_path()`. None of them depend on a line-editor crate.
  - `src/repl/notes.rs` — structured `CmdOutcome::Note { primary, badges, value }` formatters per command (`spawned`, `closed`, `send_text`, `send_key`, `wait_match`, `wait_stable`, `turn_complete`, `transcript`). The `Note::to_line()` helper renders into a ratatui `Line` for the log pane; `Note::render()` still emits an ANSI string for tests. `wait` and `turn` times are measured around the blocking RPC.
  - `src/repl/tui.rs`, `src/repl/app.rs`, `src/repl/render.rs`, `src/repl/dispatcher.rs` — **ratatui-based full-screen REPL**. `tui.rs` is the entry point: it builds the transport-aware app, enters the alternate screen via `ratatui::try_init()` (which installs a panic hook that restores the terminal on unwind), and runs the main event loop (`draw → drain UI events → poll crossterm events → handle`). `app.rs` owns mutable state: tabs/focus (via `ReplCtx`), command log, [`tui_input::Input`] buffer, completion popup, history navigation cursor, latest snapshot per adapter. `render.rs` is pure: tab strip on top, snapshot pane (~55% of remaining height), command log, input box (3-row border). `dispatcher.rs` is the background thread that subscribes to `RpcClient::notifications()`, drops `session.output` / `session.changed` from the log (`session.changed` triggers an `adapter.snapshot` fetch for the focused adapter instead), keeps `session.exited` + unknown plugin notifications, and forwards survivors as `UiEvent::Notice` / `UiEvent::Snapshot` into the main loop. **No application-specific identifiers live in any of these modules** — the REPL is a client of the generic `adapter.*` surface.
- Tests:
  ...
  - `tests/repl_tests.rs` — gated `#[cfg(feature = "repl")]`. Drives `RpcClient` + `command::dispatch` against an in-process `serve_ndjson_with_state` over pipes: capabilities, full spawn→state→close cycle (asserting the `spawned · …` note formatter fires), raw `:rpc` passthrough, adapter-flavored `session.changed` notifications. App-level coverage (input/completion/history wiring + key handling) lives in `src/repl/app.rs::tests`; per-command note formatters in `src/repl/notes.rs`; layout sizing in `src/repl/render.rs::tests`; dispatcher filtering in `src/repl/dispatcher.rs::tests`.
...
**Headless terminal geometry.** TUI classifiers usually depend on line wrapping — the host's last-resort `rows = 40, cols = 120` is *not* a safe default for any non-trivial plugin. Declare a classifier-stable preset in your `DefaultTarget` (`rows` / `cols`) so `adapter.start` picks it up when callers omit geometry. The built-in `claude-code` plugin ships **`rows = 60, cols = 200`** — wide enough that the status bar, `❯` / `>` prompt glyphs, "Total cost: …" usage screen, and the longest tool-use status lines never wrap. New TUI plugins should pick a similar preset that comfortably fits the longest unwrapped line their classifier reads — `cols >= 160`, `rows >= 40` is a reasonable starting point; tighten only after you have classifier fixtures that prove the smaller size is stable.
