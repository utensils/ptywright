...
- REPL client (gated `#[cfg(feature = "repl")]`, on by default — opt out with `cargo build --no-default-features`):
  ...
  - `src/repl/ctx.rs` — shared `ReplCtx` (tabs, focus) used by the dispatcher, the completer, and the live-pane redraw thread.
  ...
  - `src/repl/notes.rs` — structured `CmdOutcome::Note { primary, badges, value }` formatters per command (`spawned`, `closed`, `send_text`, `send_key`, `wait_match`, `wait_stable`, `turn_complete`, `transcript`). Replaces the old "dump raw RPC JSON as the `↳` line" behavior with the polished notes the homepage `OperatorHome.vue` mockup features. `wait` and `turn` times are measured around the blocking RPC.
  - `src/repl/live.rs` — **always-on live screen pane** (alt-screen + DECSTBM scroll region). Reedline doesn't natively support a regioned terminal, so this module owns the coexistence layer: enters the alternate screen at REPL startup, sets a DECSTBM scrolling region to confine reedline's drawing to the bottom rows, paints the tab strip + focused snapshot + divider into the top region with absolute `MoveTo` cursor moves bracketed by DEC save/restore cursor. `LiveLayout::compute` falls back to plain line-mode rendering on terminals smaller than 40×20. A background `spawn_redraw_thread` debounces `session.changed` notifications (30 ms) and re-pulls the focused adapter's snapshot. `AltScreenGuard` restores the terminal cleanly on Drop including on panic. No claude-code identifiers — the renderer consumes a generic `ScreenSnapshot` and reads opaque plugin name + state label strings from `ReplCtx`.
  - `src/repl/tui.rs` — **reedline-based REPL inside the live-pane shell**. The read-eval-print loop wires reedline + the notification printer + the live-pane redraw thread together: reedline draws in the bottom region courtesy of the DECSTBM split; the notification printer filters `session.output` / `session.changed` (absorbed by the live pane) and forwards `session.exited` + plugin notifications via reedline's `ExternalPrinter`; each command boundary kicks a refresh on the live-pane channel. `CmdOutcome` rendering routes `Line` / `Json` / `Note` / `Screen` to their respective painters. **No application-specific identifiers live in any of these modules** — the REPL is a client of the generic `adapter.*` surface.
- Tests:
  ...
  - `tests/repl_tests.rs` — gated `#[cfg(feature = "repl")]`. Drives `RpcClient` + `command::dispatch` against an in-process `serve_ndjson_with_state` over pipes: capabilities, full spawn→state→close cycle (asserting the `spawned · …` note formatter fires), raw `:rpc` passthrough, adapter-flavored `session.changed` notifications. Live-pane layout + paint primitives have direct unit coverage in `src/repl/live.rs` and per-command note formatters in `src/repl/notes.rs`.
