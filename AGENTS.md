...
- REPL client (gated `#[cfg(feature = "repl")]`, on by default — opt out with `cargo build --no-default-features`):
  ...
  - `src/repl/lua.rs` — embedded Lua 5.4 evaluator (`mlua`). Owns the long-lived VM, binds every REPL-exposed global (`plugins`, `session`, `send`, `wait`, `matches`, `screen_stable`, `cancel_wait`, `turn`, `state`, `transcript`, `screen`, `view`, `inspect`, `re`, `ms`, `s`), loads bundled `inspect.lua` for pretty-printing, hosts the `LuaValidator` that drives reedline's multi-line continuation. Intent names like `send_prompt` / `key` / `wait_turn_matcher` are string literals here only — they flow to plugins through the generic `adapter.send` / `adapter.wait` surface.
  - `src/repl/inspect.lua` — vendored kikito/inspect.lua 3.1.0 (MIT). Pretty-printer for return values. Loaded once at REPL startup via `include_str!` and stashed in the Lua registry.
  - `src/repl/meta.rs` — `:`-prefixed command parsing and dispatch (`:tabs`, `:focus`, `:live`, `:attach`, `:notifications`, `:rpc`, `:help`, `:quit`). Meta commands intentionally stay out of Lua so a script can't programmatically mutate REPL focus or notification subscriptions.
