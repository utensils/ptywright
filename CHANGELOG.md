...
- `ptywright repl` now subscribes to `session.changed` / `session.exited` notifications by default and prints them inline above the prompt via reedline's `external_printer`. A 300 ms heartbeat keeps the server's per-connection notification pump warm so events from sibling connections actually flush to an idle REPL.
