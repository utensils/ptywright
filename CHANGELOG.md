...
- `claude-code` plugin now exposes `wait_cancel_settled_matcher` — a thin alias over `screen_stable(completed_turn_stable_ms)` that callers can pass to `adapter.wait` after `cancel`. Mirrors the exact threshold the classifier uses to release its `cancelling` hold-state, so the wait fires the moment `adapter.state` would stop reporting `cancelling`.
