...
The manifest's `entrypoint` is resolved relative to the manifest file's parent directory, so a manifest at `/foo/plugins/echo/manifest.toml` with `entrypoint = "main.lua"` loads `/foo/plugins/echo/main.lua`. Absolute paths and `..` traversal components in `entrypoint` are rejected.
